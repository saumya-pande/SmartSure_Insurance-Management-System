...
const CHART_COLORS = ["#fa4d56","#82cfff", "#575fcf"];
...
function toChartData(entries) {
  return entries
    .map((entry, index) => ({
      ...
      color: CHART_COLOR_BY_LABEL[entry.label] || CHART_COLORS[index % CHART_COLORS.length],
    }))
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-center">
        <svg viewBox="0 0 120 120" className="w-48 h-48" role="img" aria-label={title}>
          ...
          {segments.length === 1 ? (
            <circle cx="60" cy="60" r="54" fill={segments[0].color} />
          ) : (
